...
private:
	...
	void displayUserCart(const Customer& customer) const {
	...
	int getItemAmount(const std::regex& validationRegex) const {
		std::string input;
		std::cout << "How many: ";
		std::getline(std::cin, input);
	...
	void run() {
		...
		while (programState) {
			while (mainMenu.active) {
				...
				int requestedAmount = getItemAmount(inputValidationRegex);
				
				mainMenu.items[selectedOption].quantity -= requestedAmount;
				// Perform actions based on the selected product (e.g., add to cart, update quantity, etc.)
				
				customer.cart.push_back(selectedProduct);
